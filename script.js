...
let coverImgSrc = 'CoverImage.png';
try {
    const saved = localStorage.getItem('derek_portfolio_state_v1');
    if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed && parsed.coverData) {
            if (parsed.coverData.imgSrc) coverImgSrc = parsed.coverData.imgSrc;
            if (parsed.coverData.title) {
                const titleEl = document.querySelector('h1');
                if (titleEl) titleEl.textContent = parsed.coverData.title;
            }
            if (parsed.coverData.btnText) {
                const btnEl = document.getElementById('exploreBtn');
                if (btnEl) btnEl.textContent = parsed.coverData.btnText;
            }
        }
    }
} catch (e) {
    console.warn('Could not read saved coverData:', e);
}
